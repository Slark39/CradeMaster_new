...
function TVChartHeaderInfoMobile({ col, backCol }: Props) {
...
function TVChartHeaderInfoDesktop({ col, backCol }: Props) {
  const { chartToken } = useSelector(selectChartToken);
  const { isSwap } = useSelector(selectTradeboxTradeFlags);
  ...
  return (
    <div className="Chart-header mb-10 rounded-4" style={{ color: `${col}`, backgroundColor: `${backCol}` }}>
      ...
      <div className="ExchangeChart-info VersionSwitch-wrapper">{/* <VersionSwitch /> */}</div>
    </div>
  );
}

export function TVChartHeader({ isMobile, col, backCol }: { isMobile: boolean; col: string; backCol: string }) {
  return isMobile ? (
    <TVChartHeaderInfoMobile col={col} backCol={backCol} />
  ) : (
    <TVChartHeaderInfoDesktop col={col} backCol={backCol} />
  );
}
