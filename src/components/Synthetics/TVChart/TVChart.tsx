...
import { useOracleKeeperFetcher_bsc } from "domain/synthetics/tokens/useOracleKeeperFetcher_bsc";
import { useOracleKeeperFetcher_1 } from "domain/synthetics/tokens/useOracleKeeperFetcher_1";
import { useOracleKeeperFetcher_2 } from "domain/synthetics/tokens/useOracleKeeperFetcher_2";
...
export function TVChart({ chainId, symbol, color, linestyle, col, backCol }: Props) {
  ...
  const oracleKeeperFetcher =
    chainId == 56
      ? useOracleKeeperFetcher_bsc(chainId)
      : linestyle == 2
        ? useOracleKeeperFetcher(chainId)
        : linestyle == 3
          ? useOracleKeeperFetcher_1(chainId)
          : useOracleKeeperFetcher_2(chainId);
  ...
  const { datafeed } = useTVDatafeed({ chainId, dataProvider });
